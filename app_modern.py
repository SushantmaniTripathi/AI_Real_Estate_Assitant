...
# LangChain imports - handle both 0.3.x and 1.0.x versions
try:
...
def initialize_session_state():
    ...
    if "selected_provider" not in st.session_state or st.session_state.selected_provider == "openai":
        st.session_state.selected_provider = "google"  # Default to free Google Gemini

    if "selected_model" not in st.session_state or st.session_state.selected_model == "gpt-4o-mini":
        st.session_state.selected_model = "gemini-1.5-flash"  # Fast, free Google model
...
def create_hybrid_agent_instance() -> HybridPropertyAgent:
    """Create or retrieve hybrid agent instance."""
    ...
    try:
        ...
        # Initialize LLM via provider factory (using class method directly)
        llm = ModelProviderFactory.create_model(
            model_id=model_id,
    ...
    if prompt := st.chat_input("Ask about properties..."):
        ...
        with st.chat_message("assistant"):
            ...
            try:
                ...
                if use_hybrid:
                    # Check if provider or model has changed - if so, invalidate cached agent
                    current_provider = st.session_state.selected_provider
                    current_model = st.session_state.selected_model
                    
                    if (hasattr(st.session_state, 'last_provider') and 
                        (st.session_state.last_provider != current_provider or 
                         st.session_state.last_model != current_model)):
                        print(f"DEBUG: Provider/Model changed. Clearing cached agent.")
                        st.session_state.hybrid_agent = None
                    
                    # Store current provider/model for next comparison
